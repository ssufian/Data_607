...
After shifting the values and logging it, Ozone is now more normally distributed; its definitely a step in the right
...
The after Q-Q plot showed a more normal distribution set than before, especially at the left tail end.  However, the 

middle can be further improved for sure, and that is left for future exercise
...
The after Q-Q plot showed a more normal distribution set than before, especially the middle portion is above the 45

degree line.  However again like Ozone, the left can be improved further, and that is left for future exercise
...
This was a short exercise to use an alternative "Wide to Long" method using the reshape2 libraries; melt and dcast.

The methodologies are very similar if one has multiple columns to compress into.  The 2nd half of the exercise is 

a quick statistical take on how to view the data from a statistical stand point and perform some very standard data

cleansing procedures such as data imputations of NAs, outlier detections using a capping method and some data 

transformations.  It was found through these sort of exercise that one can see how much more cleaning certain 

variables needed.  For example, Ozone and Solar Radiation had more cleanning to do even after outlier capping and 

data transformations.  The reason could be that simply filling in NAs with zeros may not have been such a good 

idea but another method of using mean or even percentile of the original data could've been better.  However, the 

point of this exercise is to learn the data one has and go through an iterative process to clean it and stage it
